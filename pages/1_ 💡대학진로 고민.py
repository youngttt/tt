...
def main():
    ...
    # 과목별 점수 입력
    korean = st.number_input("국어 등급", min_value=0, max_value=100)
    english = st.number_input("영어 등급", min_value=0, max_value=100)
    math = st.number_input("수학 등급", min_value=0, max_value=100)
    science = st.number_input("과학 등급", min_value=0, max_value=100)
    social = st.number_input("사회 등급", min_value=0, max_value=100)
    information = st.number_input("정보 등급", min_value=0, max_value=100)
    his = st.number_input("한국사 등급", min_value=0, max_value=100)

    # '평균 계산' 버튼 추가
    if st.button('평균 등급 계산🕹️'):
        # 입력된 점수들의 리스트
        scores = [korean, english, math, science, social, information, his]
        # 평균 계산
