...
if st.button('수시 합격률'):
    ...
    plt.title('dch univ the ratio of successful appllicants')  # 차트 제목 설정
...
if st.button('지원 빈도수'):
    ...
    plt.pie(counts, labels=counts.index, autopct='%1.1f%%', startangle=90)
    plt.axis('equal')  # 원형 차트를 그리기 위해 y축의 비율을 같게 설정
    plt.title('univ group frequency')  # 차트 제목 설정
...
def draw_kemss(df):
    ...
    plt.title("5 subjects grade by univ according to pass or Fail")
    st.pyplot(plt)
...
plt.title("x: korean/english/math grade, y: univ group")
st.pyplot(plt)
...
plt.title("only pass students data set ")
st.pyplot(plt)
